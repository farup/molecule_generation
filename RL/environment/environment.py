...
def _atom_addition(state, atom_types, atom_valences, atoms_with_free_valence):
  ...
  """
    ...
    atom_addition = set()
    for i in bond_order:
        for atom in atoms_with_free_valence[i]: # atom_dix form molecule
            for element in atom_types: # possible additions
...
class QEDRewardMolecule(Molecule):
    ...
    """
        ...
        return qed * self.discount_factor ** (self.max_steps - self.num_steps_taken) # discount less and less
    

class RLMol(QEDRewardMolecule): 
    def __init__(self, discount_factor, **kwargs):
        super().__init__(discount_factor, **kwargs)
        self._counter = 0
...
if __name__ == "__main__":
    ...
    get_valid_actions(state, atom_types, allow_removal, allow_no_modification, allowed_ring_sizes, allow_bonds_between_rings)


    mol = Molecule(atom_types,init_mol=None, allow_removal=True, allow_no_modification=True,
        allow_bonds_between_rings=True,
        allowed_ring_sizes=None,
        max_steps=10,
        target_fn=None,
